...
class DiffWMAttacker:
    def __init__(self, model="Manojb/stable-diffusion-2-1-base", noise_step=60, device="cuda"):
        self.pipe = ReSDPipeline.from_pretrained(model, torch_dtype=torch.float16)
    ...
    def attack(self, image):
        ...
        with torch.no_grad():
            latents = self.pipe.vae.encode(
                img_t.to(device=self.device, dtype=torch.float16)
            ).latent_dist
            latents = latents.sample(generator) * 0.18215
            noise = torch.randn([1, 4, img_t.shape[-2]//8, img_t.shape[-1]//8], device=self.device)
        ...
        return result[0][0]
attacker = DiffWMAttacker(noise_step=200, device="cuda")
...
def image_distortion(img,seed, args):
    if args.diff_noise is not None:
        attacker.noise_step = args.diff_noise
        img=attacker.attack(img)
...
def measure_diveristy(images, prompt, model, clip_preprocess, tokenizer, device):
    with torch.no_grad():
        img_batch = [clip_preprocess(i).unsqueeze(0) for i in images]
        try:
            img_batch = torch.concatenate(img_batch).to(device)
